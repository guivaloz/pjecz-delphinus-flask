...
def datatable_json():
    """DataTable JSON para listado de Ingresos"""
...
@udp_ingresos.route("/udp_ingresos/<int:udp_ingreso_id>")
def detail(udp_ingreso_id):
    """Detalle de un Ingreso"""
    udp_ingreso = UdpIngreso.query.get_or_404(udp_ingreso_id)
    return render_template("udp_ingresos/detail.jinja2", udp_ingreso=udp_ingreso)
...
@udp_ingresos.route("/udp_ingresos/edicion/<int:udp_ingreso_id>", methods=["GET", "POST"])
@permission_required(MODULO, Permiso.MODIFICAR)
def edit(udp_ingreso_id):
    """Editar Ingreso"""
    udp_ingreso = UdpIngreso.query.get_or_404(udp_ingreso_id)
    form = UdpIngresoForm()
    if form.validate_on_submit():
        udp_ingreso.ocupacion = safe_string(form.ocupacion.data, save_enie=True)
        udp_ingreso.ingresos = form.ingresos.data
        udp_ingreso.observaciones = safe_string(form.observaciones.data, save_enie=True, max_len=1024)
        udp_ingreso.save()
        bitacora = Bitacora(
            modulo=Modulo.query.filter_by(nombre=MODULO).first(),
            usuario=current_user,
            descripcion=safe_message(f"Editado ingreso de {udp_ingreso.udp_persona.nombre_completo}"),
            url=url_for("udp_personas.detail", udp_persona_id=udp_ingreso.udp_persona_id),
        )
        bitacora.save()
        flash(bitacora.descripcion, "success")
        return redirect(bitacora.url)
    form.ocupacion.data = udp_ingreso.ocupacion
    form.ingresos.data = udp_ingreso.ingresos
    form.observaciones.data = udp_ingreso.observaciones
    return render_template("udp_ingresos/edit.jinja2", form=form, udp_ingreso=udp_ingreso)


@udp_ingresos.route("/udp_ingresos/eliminar/<int:udp_ingreso_id>")
@permission_required(MODULO, Permiso.ADMINISTRAR)
def delete(udp_ingreso_id):
    """Eliminar Ingreso"""
    udp_ingreso = UdpIngreso.query.get_or_404(udp_ingreso_id)
    if udp_ingreso.estatus == "A":
        udp_ingreso.delete()
        bitacora = Bitacora(
            modulo=Modulo.query.filter_by(nombre=MODULO).first(),
            usuario=current_user,
            descripcion=safe_message(f"Eliminado ingreso de {udp_ingreso.udp_persona.nombre_completo}"),
            url=url_for("udp_personas.detail", udp_persona_id=udp_ingreso.udp_persona_id),
        )
        bitacora.save()
        flash(bitacora.descripcion, "success")
    return redirect(url_for("udp_personas.detail", udp_persona_id=udp_ingreso.udp_persona_id))


@udp_ingresos.route("/udp_ingresos/recuperar/<int:udp_ingreso_id>")
@permission_required(MODULO, Permiso.ADMINISTRAR)
def recover(udp_ingreso_id):
    """Recuperar Ingreso"""
    udp_ingreso = UdpIngreso.query.get_or_404(udp_ingreso_id)
    if udp_ingreso.estatus == "B":
        udp_ingreso.recover()
        bitacora = Bitacora(
            modulo=Modulo.query.filter_by(nombre=MODULO).first(),
            usuario=current_user,
            descripcion=safe_message(f"Recuperado ingreso de {udp_ingreso.udp_persona.nombre_completo}"),
            url=url_for("udp_personas.detail", udp_persona_id=udp_ingreso.udp_persona_id),
        )
        bitacora.save()
        flash(bitacora.descripcion, "success")
